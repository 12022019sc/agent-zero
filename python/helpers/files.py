...
def fix_dev_path(path: str):
    "On dev environment, convert /home/shayne/agent-zero/... paths to local absolute paths"
    from python.helpers.runtime import is_development

    if is_development():
        if path.startswith("/home/shayne/agent-zero/"):
            path = path.replace("/home/shayne/agent-zero/", "")
    return get_abs_path(path)


def normalize_a0_path(path: str):
    "Convert absolute paths into /home/shayne/agent-zero/... paths"
    if is_in_base_dir(path):
        deabs = deabsolute_path(path)
        return "/home/shayne/agent-zero/" + deabs
    return path
